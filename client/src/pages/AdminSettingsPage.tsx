...
import { Shield, ToggleLeft, ToggleRight, AlertTriangle, RefreshCw } from "lucide-react";
    ...
    <div className="flex items-start gap-4 px-5 py-4 border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground/60 mt-1">
          Key: <code className="font-mono bg-muted px-1 rounded text-xs">{flag.flagKey}</code>
...
export default function AdminSettingsPage() {
  ...
  const { data: flags = [], refetch, isLoading } = trpc.featureFlags.getAll.useQuery();
      ...
      <div className="border-b border-border px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Manage feature access and system configuration</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Control which features are visible to the <strong>Contractor</strong> role.
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            {isLoading ? (
              <div className="px-5 py-8 text-center text-sm text-muted-foreground">Loading flags…</div>
            ) : flags.length === 0 ? (
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Changes take effect immediately — contractors will see the updated state on their next page load.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden divide-y divide-border">
            <div className="px-5 py-3 flex items-start gap-3">
              <Badge className="bg-primary/20 text-primary border-primary/30 shrink-0 mt-0.5">admin</Badge>
              <div>
                <p className="text-xs font-medium">Administrator</p>
                <p className="text-xs text-muted-foreground">Full access to all features, settings, and admin pages. Feature flags do not apply.</p>
              </div>
            </div>
            <div className="px-5 py-3 flex items-start gap-3">
              <Badge variant="outline" className="shrink-0 mt-0.5">contractor</Badge>
              <div>
                <p className="text-xs font-medium">Contractor (public user)</p>
                <p className="text-xs text-muted-foreground">Access to all standard features. Feature flags control visibility of beta/premium features.</p>
              </div>
            </div>
            <div className="px-5 py-3 flex items-start gap-3">
              <Badge variant="outline" className="text-muted-foreground shrink-0 mt-0.5">user</Badge>
              <div>
                <p className="text-xs font-medium">User (legacy)</p>
                <p className="text-xs text-muted-foreground">Legacy role, treated identically to Contractor. New signups default to this role.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card px-5 py-4">
          <p className="text-xs text-muted-foreground">
            Signed in as <strong className="text-foreground">{user?.email ?? user?.name}</strong>
