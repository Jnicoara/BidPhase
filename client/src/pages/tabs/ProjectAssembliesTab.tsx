...
export default function ProjectAssembliesTab({ projectId }: ProjectAssembliesTabProps) {
  const utils = trpc.useUtils();
  const [expandedIds, setExpandedIds] = useState<Set<number>>(() => new Set<number>());
  const [showAddModal, setShowAddModal] = useState(false);

  const { data: assemblies = [], isLoading } = trpc.projectAssemblies.list.useQuery({ projectId });
  const deleteAssembly = trpc.projectAssemblies.delete.useMutation({
    onSuccess: () => { utils.projectAssemblies.list.invalidate({ projectId }); toast.success("Assembly removed"); },
  ...
  return (
    <div className="p-5 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h3 className="text-base font-semibold text-foreground">Project Assemblies</h3>
          <p className="text-sm text-muted-foreground mt-0.5">
            Add assemblies from your master library or build custom ones for this project.
          </p>
        </div>
        <Button
      ...
      {assemblies.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-border/40 rounded-xl">
          ...
          <p className="text-sm text-muted-foreground/60 mt-1">Add assemblies from your master library or create a custom one</p>
        </div>
      ) : (
...
function AssemblyCard({
  assembly, projectId, expanded, onToggle, onDelete,
}: {
  assembly: { id: number; name: string; phase?: string | null; items?: Array<{ id: number; description: string; unit: string; qty: string | number; masterMaterialCost: string | number; masterLaborHours: string | number; overrideMaterialCost: string | number; overrideLaborHours: string | number }> };
  projectId: number;
  expanded: boolean;
  onToggle: () => void;
  onDelete: () => void;
  ...
  const resetItem = trpc.projectAssemblies.resetItem.useMutation({
    onSuccess: () => { utils.projectAssemblies.list.invalidate({ projectId }); toast.success("Reset to master values"); },
    onError: (e) => toast.error(e.message),
  });
  ...
  type AsmItem = { overrideMaterialCost?: string | number | null; masterMaterialCost?: string | number | null; overrideLaborHours?: string | number | null; masterLaborHours?: string | number | null; qty?: string | number | null };
  const totalMat = (items as AsmItem[]).reduce((s, i) => s + parseFloat(String(i.overrideMaterialCost ?? i.masterMaterialCost ?? 0)) * parseFloat(String(i.qty ?? 1)), 0);
  const totalLab = (items as AsmItem[]).reduce((s, i) => s + parseFloat(String(i.overrideLaborHours ?? i.masterLaborHours ?? 0)) * parseFloat(String(i.qty ?? 1)), 0);
    ...
    <div className="border border-border/40 rounded-xl bg-card overflow-hidden">
      ...
      {expanded && (
        <div className="border-t border-border/30">
          {items.length === 0 ? (
            <p className="text-sm text-muted-foreground/60 italic px-4 py-3">No items in this assembly</p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border/30 bg-muted/20">
                    ...
                    <th className="text-center px-3 py-2 text-xs font-semibold text-muted-foreground w-20">Unit</th>
                    <th className="text-center px-3 py-2 text-xs font-semibold text-muted-foreground w-20">Qty</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-muted-foreground w-28">Mat Cost/Unit</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-muted-foreground w-28">Labor Hrs/Unit</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-muted-foreground w-28">Ext Mat</th>
                    <th className="text-right px-3 py-2 text-xs font-semibold text-muted-foreground w-28">Ext Labor</th>
                    <th className="w-20"></th>
                  </tr>
                </thead>
                <tbody>
          {(items as Array<{ id: number; description: string; unit: string; qty: string | number; masterMaterialCost: string | number; masterLaborHours: string | number; overrideMaterialCost: string | number; overrideLaborHours: string | number }>).map(item => (
            <AssemblyItemRow
              key={item.id}
              item={item}
                      onUpdateQty={(qty) => updateItem.mutate({ id: item.id, qty })}
                      onOverrideMat={(v) => updateItem.mutate({ id: item.id, overrideMaterialCost: v })}
                      onOverrideLab={(v) => updateItem.mutate({ id: item.id, overrideLaborHours: v })}
                ...
                <tfoot>
                  <tr className="border-t border-border/30 bg-muted/10">
                    <td colSpan={5} className="px-4 py-2 text-xs font-semibold text-muted-foreground text-right">Totals:</td>
                    <td className="px-3 py-2 text-right text-sm font-bold text-foreground">${totalMat.toFixed(2)}</td>
                    <td className="px-3 py-2 text-right text-sm font-bold text-foreground">{totalLab.toFixed(2)}</td>
                    <td />
                  </tr>
                </tfoot>
              </table>
...
function AssemblyItemRow({
  item, onUpdateQty, onOverrideMat, onOverrideLab, onReset, onDelete,
}: {
  item: { id: number; description: string; unit: string; qty: string | number; masterMaterialCost: string | number; masterLaborHours: string | number; overrideMaterialCost: string | number; overrideLaborHours: string | number };
  ...
  const overrideMat = parseFloat(String(item.overrideMaterialCost ?? masterMat));
  ...
  return (
    <tr className="border-b border-border/20 hover:bg-muted/10 transition-colors">
      ...
      <td className="px-3 py-2 text-center">
        <InlineNumberInput value={qty} onCommit={onUpdateQty} min={0} step={1} className="w-16 text-center" />
      </td>
      <td className="px-3 py-2 text-right">
        <InlineNumberInput
          ...
          min={0}
          step={0.01}
          className={cn("w-24 text-right", matOverridden && "text-yellow-400 border-yellow-500/40")}
          prefix="$"
        />
      </td>
      <td className="px-3 py-2 text-right">
        <InlineNumberInput
          ...
          min={0}
          step={0.001}
          className={cn("w-24 text-right", labOverridden && "text-yellow-400 border-yellow-500/40")}
        />
      </td>
      <td className="px-3 py-2 text-right text-foreground font-medium">${(overrideMat * qty).toFixed(2)}</td>
      <td className="px-3 py-2 text-right text-foreground font-medium">{(overrideLab * qty).toFixed(3)}</td>
...
function InlineNumberInput({
  value, onCommit, min, step, className, prefix,
}: {
  value: number;
  ...
  if (!editing) {
    return (
      ...
      >
        {prefix}{value % 1 === 0 ? value.toFixed(0) : value.toFixed(step && step < 0.01 ? 3 : 2)}
      </button>
    );
  }
  ...
  return (
    <input
      ...
      onKeyDown={e => {
        if (e.key === "Enter") { const n = parseFloat(draft); if (!isNaN(n)) onCommit(n); setEditing(false); }
...
function AddAssemblyModal({
  ...
  const addFromMaster = trpc.projectAssemblies.addFromMaster.useMutation({
    onSuccess: () => { toast.success("Assembly added"); onAdded(); },
    onError: (e) => toast.error(e.message),
  });

  const createBlank = trpc.projectAssemblies.createBlank.useMutation({
    onSuccess: () => { toast.success("Assembly created"); onAdded(); },
    onError: (e) => toast.error(e.message),
  });
      ...
      <DialogContent className="max-w-md bg-card border-border/60">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            {(["master", "blank"] as const).map(m => (
              <button
                ...
                className={cn(
                  ...
                  mode === m ? "bg-[#F5C518] text-black" : "text-muted-foreground hover:text-foreground"
                )}
              >
            ...
            <div className="space-y-1.5">
              ...
              {masterAssemblies.length === 0 ? (
                <p className="text-sm text-muted-foreground/60 italic">
                  No master assemblies yet. Create them in Settings → Master Library.
                </p>
              ) : (
          ...
          <div className="flex gap-3 pt-1">
            ...
            <Button
              type="submit"
              disabled={isPending || (mode === "master" && !selectedMasterId) || (mode === "blank" && !blankName.trim())}
              className="flex-1 bg-[#F5C518] hover:bg-[#e6b800] text-black font-semibold"
            >
              {isPending ? <Loader2 size={16} className="animate-spin" /> : "Add Assembly"}
            </Button>
          </div>
        </form>
      </DialogContent>
