...
import {
  ArrowLeft, Pencil, Check, X, ChevronDown,
  Loader2, Calendar, User, MapPin, FileText,
...
export default function ProjectDetailPage({ projectId, onBack, onOpenMaterialList }: ProjectDetailPageProps) {
  const [activeTab, setActiveTab] = useState<DetailTab>("estimating");

  const { data: project, isLoading, refetch } = trpc.projects.get.useQuery({ id: projectId });
  const updateProject = trpc.projects.update.useMutation({
    onSuccess: () => { refetch(); toast.success("Project updated"); },
  ...
  const saveField = (field: string) => {
    ...
    updateProject.mutate({ id: projectId, [field]: val || null } as Parameters<typeof updateProject.mutate>[0]);
  ...
  const renderEstimatingTab = () => {
    ...
    const cat = (project.category ?? "commercial") as "civil" | "commercial" | "residential" | "industrial";
    return (
      <UnifiedProjects
        category={cat}
      />
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            {editingField === "name" ? (
              <div className="flex items-center gap-2">
                <Input
                  ...
                  autoFocus
                  onKeyDown={e => { if (e.key === "Enter") saveField("name"); if (e.key === "Escape") cancelEdit(); }}
                />
                <button onClick={() => saveField("name")} className="text-green-400 hover:text-green-300 p-1"><Check size={16} /></button>
                <button onClick={cancelEdit} className="text-muted-foreground hover:text-foreground p-1"><X size={16} /></button>
              </div>
            ) : (
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1.5">
              {/* Status */}
              <Select value={project.status ?? "Bidding"} onValueChange={v => saveStatus(v as ProjectStatus)}>
      ...
      <div className="flex-1 min-h-0 overflow-auto">
        ...
        {activeTab === "assemblies"  && <ProjectAssembliesTab projectId={projectId} />}
...
function EditableMetaField({
  icon, value, placeholder, field,
  editingField, editValues, onStart, onSave, onCancel, onChange,
}: {
  ...
  if (editingField === field) {
    return (
      <div className="flex items-center gap-1">
        ...
        />
        <button onClick={() => onSave(field)} className="text-green-400 p-0.5"><Check size={13} /></button>
        <button onClick={onCancel} className="text-muted-foreground p-0.5"><X size={13} /></button>
      </div>
    );
  }
  return (
    <button
      ...
      className={cn(
        ...
        value ? "text-muted-foreground hover:text-foreground" : "text-muted-foreground/40 hover:text-muted-foreground"
      )}
    >
      {icon}
      <span>{value ?? placeholder}</span>
      <Pencil size={11} className="opacity-0 group-hover/meta:opacity-60 transition-opacity" />
    </button>
  );
}

function EditableDateField({
  value, editingField, editValues, onStart, onSave, onCancel, onChange,
}: {
  ...
  return (
    <button
      ...
      className={cn(
        "flex items-center gap-1.5 text-sm transition-colors group/meta",
        displayDate ? "text-muted-foreground hover:text-foreground" : "text-muted-foreground/40 hover:text-muted-foreground"
      )}
    >
      <Calendar size={13} />
