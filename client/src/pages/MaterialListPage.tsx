...
function expandRunToRows(
  ...
  if (!isWireRun) {
    // ── Conduit row ──────────────────────────────────────────────────────────
    const conduitBillable = calcConduitBillable(
      r.feet,
      r.conduitWasteFactor ?? 10
    );
    ...
    if (!r.conduitOnly) {
      ...
      const effectiveWireSize =
        wireTypeStr === "MC" || wireTypeStr === "NM"
          ? (r.wireTypeId?.match(/-([\d/]+(?:-\d+)?)(?:-[a-z])?$/i)?.[1] ??
            r.conductorSize ??
            "12")
          : (r.conductorSize ?? "12");
      const wireBillable = calcConduitWire(
        r.feet,
        r.conductors,
        r.wireTermMakeup ?? 0,
        r.numPullPoints ?? 0,
      ...
      // ── EGC row ──────────────────────────────────────────────────────────────
      if (r.includeGround) {
        const egcBillable = calcConduitWire(
          r.feet,
          1,
          r.wireTermMakeup ?? 0,
          r.numPullPoints ?? 0,
        ...
        const egcCpf = getWirePricePerFoot(
          "thhn",
          r.groundSize ?? "12",
          r.groundMaterial ?? "CU",
          userMaterials,
...
function InlineEdit({
  ...
  if (!editing) {
    return (
      <span
        className={cn(
          "cursor-pointer hover:text-[#F5C518] transition-colors group flex items-center gap-1",
          className
        )}
        onClick={() => {
          setDraft(value);
  ...
  return (
    <input
      ...
      className={cn(
        "bg-transparent border-b border-[#F5C518] outline-none text-[#F5C518]",
        inputClassName
      )}
    />
  );
...
function CatalogAddModal({
  ...
  return (
    ...
    >
      <div
        className="bg-card border border-border rounded-xl w-[560px] max-h-[80vh] flex flex-col shadow-2xl"
          ...
          {filtered.map(item => (
            ...
            >
              <div className="flex flex-col gap-0.5 min-w-0">
                <span className="text-xs font-medium text-foreground group-hover:text-[#F5C518] transition-colors truncate">
                  {item.description}
                </span>
              ...
              <div className="flex items-center gap-3 shrink-0 ml-4">
                ...
                <Plus
                  size={12}
                  className="text-muted-foreground group-hover:text-[#F5C518] transition-colors"
                />
              </div>
            </button>
          ))}
...
export default function MaterialListPage({ onBack }: MaterialListPageProps) {
    ...
    const addProjectRows = (proj: CivilProject | undefined, prefix: string) => {
      ...
      for (const smr of st.savedMaterialRows ?? []) {
        ...
        allRows.push({
          ...
          unit: smr.unit,
          qty: smr.qty,
          unitCost: smr.unitCost,
          notes: `Saved ${new Date(smr.savedAt).toLocaleDateString()}`,
          catalogId: null,
          source: "session",
        });
      }
      ...
      const savedSessionIds = new Set(
        (st.savedMaterialRows ?? []).map(r => (r as SavedMaterialRow).sessionId)
      );
  ...
  const applyCatalog = (rowId: string, item: CatalogItem | null) => {
    if (!item) {
      updateRow(rowId, { catalogId: null });
      return;
    }
    updateRow(rowId, {
      catalogId: item.id,
  ...
  const addJLine = () =>
    setJourneymanLines([
      ...journeymanLines,
      { id: nanoid(6), description: "Task", hours: 0 },
    ]);
  const addTLine = () =>
    setTraineeLines([
      ...traineeLines,
  ...
  const updateJLine = (id: string, patch: Partial<LaborLine>) =>
    setJourneymanLines(
      journeymanLines.map(l => (l.id === id ? { ...l, ...patch } : l))
    );
  const updateTLine = (id: string, patch: Partial<LaborLine>) =>
    setTraineeLines(
      traineeLines.map(l => (l.id === id ? { ...l, ...patch } : l))
    );
  ...
  const exportCSV = () => {
    const lines = [
      ...
      ...allRows.map(
        r =>
          `"${r.source}","${r.description}",${r.unit},${r.qty},${r.unitCost.toFixed(2)},${(r.qty * r.unitCost).toFixed(2)},"${r.notes}"`
      ),
      "",
      "Type,Description,Hours,Rate,Total",
      ...journeymanLines.map(
        l =>
          `Journeyman,"${l.description}",${l.hours},${journeymanRate.toFixed(2)},${(l.hours * journeymanRate).toFixed(2)}`
      ),
      ...traineeLines.map(
        l =>
          `Trainee,"${l.description}",${l.hours},${traineeRate.toFixed(2)},${(l.hours * traineeRate).toFixed(2)}`
      ),
      "",
  ...
  const exportPDF = () => {
    ...
    if (!w) return;
    w.document
      .write(`<!DOCTYPE html><html><head><title>Labor & Material Summary</title>
    ...
    <div className="flex flex-col h-full w-full bg-background overflow-hidden">
      ...
      <header className="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-border bg-card/80 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs border border-border text-muted-foreground hover:text-foreground hover:border-[#F5C518]/40 transition-all"
          >
            <Printer size={13} />
            Print
          </button>
          ...
          <button
            onClick={exportPDF}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-[#F5C518] text-black font-medium hover:bg-[#F5C518]/90 transition-all"
          >
            <FileText size={13} />
            PDF
          </button>
        ...
        <section className="bg-card border border-border rounded-xl">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground font-mono">
                {allRows.length} items · ${fmt(materialSubtotal)}
              </span>
            </div>
            {showMaterials ? (
          ...
          {showMaterials && (
            <div>
              ...
              {allRows.length === 0 && (
                <div className="px-4 py-8 text-center text-xs text-muted-foreground">
                  No materials yet. Add count sessions or click "+ Add Row".
                </div>
              )}
              {allRows.length > 0 && visibleRows.length === 0 && (
                <div className="px-4 py-6 text-center text-xs text-muted-foreground">
                  No rows match "{materialSearch}".
                </div>
              )}
              {visibleRows.map(row => (
                <div
                  ...
                  className={cn(
                    ...
                    selectedIds.has(row.id) &&
                      "bg-[#F5C518]/5 border-l-2 border-l-[#F5C518]/40"
                  )}
                >
                  <button
                  ...
                  >
                    {selectedIds.has(row.id) ? (
                      <CheckSquare size={12} className="text-[#F5C518]" />
                    ) : (
                      <Square size={12} />
                    )}
                  </button>
                  <div className="flex flex-col gap-1 min-w-0">
                    ...
                    <CatalogPicker
                      value={row.catalogId}
                      onChange={item => applyCatalog(row.id, item)}
                      placeholder="Link to catalog…"
                    />
                  </div>
                  <InlineEdit
                    value={row.unit}
                    onSave={v => updateRow(row.id, { unit: v })}
                    className="text-xs text-muted-foreground font-mono mt-1"
                    inputClassName="text-xs w-14 font-mono"
                  />
                  <div className="text-right mt-1">
                    <InlineEdit
                      value={String(row.qty)}
                      onSave={v =>
                        updateRow(row.id, { qty: parseFloat(v) || 0 })
                      }
                      className="text-xs font-mono text-foreground justify-end"
                      inputClassName="text-xs w-14 font-mono text-right"
                    />
                  </div>
                  <div className="text-right mt-1">
                    <InlineEdit
                      ...
                      onSave={v =>
                        updateRow(row.id, { unitCost: parseFloat(v) || 0 })
                      }
                      className="text-xs font-mono text-foreground justify-end"
                      inputClassName="text-xs w-16 font-mono text-right"
                      prefix="$"
                    />
                  </div>
                  <div className="text-right text-xs font-mono font-medium text-[#F5C518] mt-1">
                    ${fmt(row.qty * row.unitCost)}
                  </div>
                  <InlineEdit
                    value={row.notes}
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-red-400 mt-1"
                  >
                    <Trash2 size={12} />
                  </button>
                </div>
              ...
              <div className="px-4 py-2 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-[#F5C518] transition-colors"
                  >
                    <Plus size={12} />
                    Add Row
                  </button>
        ...
        <section className="bg-card border border-border rounded-xl">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <input
                  ...
                  step={1}
                  value={jRateDraft}
                  onChange={e => setJRateDraft(e.target.value)}
                  onBlur={() => {
                    const v = parseFloat(jRateDraft);
                    if (!isNaN(v) && v > 0) setJourneymanRate(v);
                    else setJRateDraft(String(journeymanRate));
                  }}
          ...
          {showJourneyman && (
            <div>
              ...
              {journeymanLines.length === 0 && (
                <div className="px-4 py-6 text-center text-xs text-muted-foreground">
                  No journeyman tasks yet.
                </div>
              )}
              {journeymanLines.map(line => (
                <div
                  key={line.id}
                  className="grid grid-cols-[1fr_100px_100px_28px] gap-2 px-4 py-2 border-b border-border/40 items-center group hover:bg-muted/10 transition-colors"
                >
                  <InlineEdit
                    value={line.description}
                    onSave={v => updateJLine(line.id, { description: v })}
                  ...
                  <div className="text-right">
                    <InlineEdit
                      ...
                      onSave={v =>
                        updateJLine(line.id, { hours: parseFloat(v) || 0 })
                      }
                      className="text-xs font-mono text-foreground justify-end"
                      inputClassName="text-xs w-20 font-mono text-right"
                    />
                  </div>
                  <div className="text-right text-xs font-mono font-medium text-[#F5C518]">
                    ${fmt(line.hours * journeymanRate)}
                  </div>
                  <button
                    onClick={() => setDeletingLaborId(line.id)}
        ...
        <section className="bg-card border border-border rounded-xl">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <input
                  ...
                  step={1}
                  value={tRateDraft}
                  onChange={e => setTRateDraft(e.target.value)}
                  onBlur={() => {
                    const v = parseFloat(tRateDraft);
                    if (!isNaN(v) && v > 0) setTraineeRate(v);
                    else setTRateDraft(String(traineeRate));
                  }}
                  className="w-14 bg-transparent border-b border-[#F5C518]/40 text-[#F5C518] font-mono text-xs text-right outline-none focus:border-[#F5C518]"
                />
                <span>/hr</span>
          ...
          {showTrainee && (
            <div>
              ...
              {traineeLines.length === 0 && (
                <div className="px-4 py-6 text-center text-xs text-muted-foreground">
                  No trainee tasks yet.
                </div>
              )}
              {traineeLines.map(line => (
                ...
                >
                  <InlineEdit
                    value={line.description}
                    onSave={v => updateTLine(line.id, { description: v })}
                    className="text-xs text-foreground"
                    inputClassName="text-xs w-full"
                  />
                  <div className="text-right">
                    <InlineEdit
                      value={String(line.hours)}
                      onSave={v =>
                        updateTLine(line.id, { hours: parseFloat(v) || 0 })
                      }
                      className="text-xs font-mono text-foreground justify-end"
                      inputClassName="text-xs w-20 font-mono text-right"
                    />
          ...
          <div className="divide-y divide-border/40">
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <input
                  ...
                  onBlur={() => {
                    ...
                    if (!isNaN(v) && v >= 0) setMarkupPct(v);
                    else setMarkupDraft(String(markupPct));
                  }}
                  className="w-14 bg-transparent border-b border-[#F5C518]/40 text-[#F5C518] font-mono text-xs text-right outline-none focus:border-[#F5C518]"
                />
                <span className="text-xs">%</span>
              </div>
              <span className="font-mono font-medium text-[#F5C518]">
                +${fmt(markupAmt)}
              </span>
            </div>
            <div className="flex items-center justify-between px-4 py-2.5 text-sm bg-muted/10">
              ...
              <span className="font-mono font-semibold">
                ${fmt(materialSubtotal + markupAmt)}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-[#F5C518]/5">
              ...
              <span className="font-mono font-bold text-xl text-[#F5C518]">
                ${fmt(grandTotal)}
              </span>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-xl p-6 w-80 shadow-2xl">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              Count session data in the plan is not affected.
            </p>
            <div className="flex gap-2 justify-end">
              ...
              <button
                onClick={bulkDelete}
                className="px-3 py-1.5 text-xs bg-red-500/90 text-white rounded-lg hover:bg-red-500 transition-colors"
              >
                Delete All
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-xl p-6 w-80 shadow-2xl">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                onClick={() => {
                  setRows(p => p.filter(r => r.id !== deletingRowId));
                  setManualRows(p => p.filter(r => r.id !== deletingRowId));
      ...
      {deletingLaborId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-xl p-6 w-80 shadow-2xl">
            <h3 className="text-sm font-semibold mb-2">
              Remove this labor task?
            </h3>
            <p className="text-xs text-muted-foreground mb-4">
              This will permanently remove the labor line.
            </p>
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setDeletingLaborId(null)}
                className="px-3 py-1.5 text-xs border border-border rounded-lg hover:bg-muted/20 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={() => {
                  setJourneymanLines(
                    journeymanLines.filter(l => l.id !== deletingLaborId)
                  );
                  setTraineeLines(
                    traineeLines.filter(l => l.id !== deletingLaborId)
                  );
