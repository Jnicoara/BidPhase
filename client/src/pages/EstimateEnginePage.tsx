  ...
  return text
    ...
    .map(line => {
      ...
      if (qtyLast)
        return {
          raw: line,
          qty: parseFloat(qtyLast[2]),
          description: qtyLast[1].trim(),
        };
      if (qtyEnd && parseFloat(qtyEnd[2]) > 0 && parseFloat(qtyEnd[2]) < 10000)
        return {
          raw: line,
          qty: parseFloat(qtyEnd[2]),
          description: qtyEnd[1].trim(),
        };
          ...
          >
            ...
            {selected ? (
              ...
              <span className="text-muted-foreground flex-1">
                Search assemblies for {activeCategory}…
              </span>
            )}
            <ChevronDown
              size={13}
              ...
              <div className="max-h-64 overflow-y-auto">
                {filtered.length === 0 ? (
                  <div className="px-3 py-4 text-xs text-muted-foreground text-center">
                    No assemblies found for {activeCategory}
                  </div>
                ) : (
                  filtered.map(a => (
                    <button
                      ...
                      className={cn(
                        ...
                        selected?.assembly_id === a.assembly_id &&
                          "bg-[#F5C518]/10"
                      )}
                    >
                      <div className="flex items-center gap-2">
                      ...
                      <div className="text-muted-foreground mt-0.5 pl-[19px] truncate">
                        {a.description}
                      </div>
...
export default function EstimateEnginePage({
  onBack,
}: EstimateEnginePageProps) {
  const {
    setShowMaterialList,
  ...
  const [crewSlider, setCrewSlider] = useState(1.0);
  const [crewDraft, setCrewDraft] = useState("1.00");
  const [laborRate, setLaborRate] = useState(85);
  const [laborRateDraft, setLaborRateDraft] = useState("85");
  const [showReport, setShowReport] = useState(false);
  const [expandedPhases, setExpandedPhases] = useState<Set<string>>(
    new Set([...DB_PHASES, "Assembly"])
  );
  const [expandedAssemblies, setExpandedAssemblies] = useState<Set<number>>(
    new Set()
  );
  ...
  const addAssemblyLine = useCallback(
    (assembly: ElectricalAssembly, qty: number) => {
  ...
  const runEstimate = useCallback(async () => {
    ...
    for (const line of assemblyLines) {
      const assembly = ELECTRICAL_ASSEMBLIES.find(
        a => a.assembly_id === line.assembly_id
      );
      ...
      for (const comp of assembly.components) {
        ...
        const childLaborHrs =
          childQty * comp.base_labor_hrs * catFactor * crewSlider;
  ...
  const matchedRows = estimateRows.filter(
    r => r.rowType === "item" && r.matched
  );
  const unmatchedRows = estimateRows.filter(
    r => r.rowType === "item" && !r.matched
  );
      ...
      if (r.rowType === "item") {
        lines.push(
          [
            ...
            r.qty,
            r.unit,
            (r.unitMaterial ?? 0).toFixed(2),
            (r.baseLaborHours ?? 0).toFixed(4),
            (r.categoryFactor ?? 1).toFixed(2),
            ((r.finalLaborHours ?? 0) * r.qty).toFixed(3),
        ...
        for (const c of r.children) {
          lines.push(
            [
              ...
              c.qty.toFixed(2),
              c.unit,
              c.unitMaterial.toFixed(2),
              c.baseLaborHrs.toFixed(4),
              "",
              c.finalLaborHrs.toFixed(3),
    ...
    >
      ...
      <header className="sticky top-0 z-30 flex items-center gap-3 px-4 py-3 border-b border-border bg-background/95 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 ml-2">
          ...
          <span className="text-xs text-violet-400 border border-violet-500/30 bg-violet-500/10 px-1.5 py-0.5 rounded font-medium">
            v2 · Dual-Mode
          </span>
        </div>
        <div className="ml-auto flex items-center gap-2 text-xs text-muted-foreground">
          <span
            className={cn(
              ...
              pricingSource === "platt" ? "text-green-400" : "text-amber-400"
            )}
          >
            <span
              className={cn(
                "w-1.5 h-1.5 rounded-full",
                pricingSource === "platt" ? "bg-green-400" : "bg-amber-400"
              )}
            />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {(
                  ...
                  >
                    ...
                    <span className="font-mono text-xs">
                      ×{(CATEGORY_MULTIPLIERS[cat] ?? 1).toFixed(2)}
                    </span>
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    onBlur={() => {
                      const v = Math.min(
                        1.5,
                        Math.max(0.6, parseFloat(crewDraft) || 1.0)
                      );
                      setCrewSlider(v);
                      setCrewDraft(v.toFixed(2));
                    }}
                ...
                <div className="text-xs text-muted-foreground border-t border-border pt-2">
                  ...
                  <span className="text-[#F5C518] font-mono">
                    {(CATEGORY_MULTIPLIERS[category] ?? 1).toFixed(2)}
                  </span>{" "}
                  ×{" "}
                  <span className="text-[#F5C518] font-mono">
                    {crewSlider.toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bp-card p-4 space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onBlur={() => {
                    const v = Math.min(
                      300,
                      Math.max(20, parseFloat(laborRateDraft) || 85)
                    );
                    setLaborRate(v);
              ...
              <p className="text-[11px] text-muted-foreground">
                Blended journeyman rate used to compute labor line totals.
              </p>
              <div className="text-xs text-muted-foreground border-t border-border pt-2 space-y-0.5">
                ...
                <div className="flex justify-between">
                  <span>Cat Factor:</span>
                  <span className="text-[#F5C518] font-mono">
                    ×{(CATEGORY_MULTIPLIERS[category] ?? 1).toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Crew Factor:</span>
                  <span className="text-[#F5C518] font-mono">
                    ×{crewSlider.toFixed(2)}
                  </span>
                </div>
                ...
                <div>
                  {sessionLines.length === 0 ? (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground py-4">
                      <Info size={14} />
                      <span>
                        No count sessions found. Open a project and create count
                        sessions, or switch to Paste Raw List.
                      </span>
                    </div>
                  ) : (
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground mb-3">
                        Items pulled from all active project count sessions and
                        saved material rows:
                      </p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 max-h-48 overflow-y-auto pr-1">
                        {sessionLines.map((line, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-muted/10 border border-border text-xs"
                          >
                            <span className="font-mono text-[#F5C518] shrink-0">
                              {line.qty}×
                            </span>
              ...
              {inputMode === "paste" && (
                <div className="space-y-2">
                  <p className="text-xs text-muted-foreground">
                    Paste one item per line. Formats:{" "}
                    <span className="font-mono text-foreground">
                      20 #12 AWG THHN Wire
                    </span>{" "}
                    or{" "}
                    <span className="font-mono text-foreground">
                      #12 AWG THHN Wire x 20
                    </span>
                  </p>
                  <textarea
                    value={pastedText}
                  ...
                  {pastedText && (
                    <p className="text-xs text-muted-foreground">
                      {parsePastedList(pastedText).length} items parsed
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  {assemblyLines.length > 0 && (
                    <div className="space-y-2">
                      <div className="text-xs text-muted-foreground font-medium uppercase tracking-wider">
                        Assembly Takeoff List
                      </div>
                      {assemblyLines.map(line => {
                        const asm = ELECTRICAL_ASSEMBLIES.find(
                          a => a.assembly_id === line.assembly_id
                        );
                        return (
                          <div
                            key={line.id}
                            className="flex items-center gap-2 px-3 py-2 rounded-md border border-violet-500/30 bg-violet-500/5 text-xs"
                          >
                            <Package
                              size={12}
                              className="text-violet-400 shrink-0"
                  ...
                  {assemblyLines.length === 0 && (
                    <div className="flex items-center gap-2 text-xs text-muted-foreground py-2">
                      <Info size={13} />
                      <span>
                        Select an assembly above and click "Add Assembly" to
                        build your takeoff list.
                      </span>
                    </div>
                  )}
                </div>
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  {totalTakeoffCount} line{totalTakeoffCount !== 1 ? "s" : ""})
                </>
              )}
            </button>
          </div>
          ...
          {showReport && estimateRows.length > 0 && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  },
                  {
                    ...
                    value: fmt(totalLaborHours, 1) + " hrs",
              ...
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1 text-green-400">
                    ...
                    {matchedRows.length} items matched
                  </span>
                  {assemblyRows.length > 0 && (
                    <span className="flex items-center gap-1 text-violet-400">
                      <Package size={12} />
                      {assemblyRows.length} assembl
                      {assemblyRows.length !== 1 ? "ies" : "y"}
                    </span>
                  )}
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={exportCSV}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-muted/20 border border-border text-xs text-muted-foreground hover:text-foreground hover:border-border/80 transition-colors"
                  >
                ...
                <div className="bp-card overflow-hidden border-violet-500/30">
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground font-mono ml-auto">
                      {rowsByPhase.get("Assembly")!.length} assembl
                      {rowsByPhase.get("Assembly")!.length !== 1 ? "ies" : "y"}
                    </span>
                    <span className="text-xs font-mono text-foreground">
                      {fmt(
                        rowsByPhase
                          .get("Assembly")!
                          .reduce((s, r) => s + (r.assemblyLaborHrs ?? 0), 0),
                        1
                      )}{" "}
                      hrs
                    </span>
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <span className="text-muted-foreground font-mono shrink-0">
                                {fmt(row.assemblyLaborHrs ?? 0, 1)} hrs
                              </span>
                              <span className="font-mono font-semibold text-violet-300 shrink-0">
                                ${fmt(row.lineTotal)}
                              </span>
                                ...
                                <table className="w-full text-xs">
                                  <thead>
                                    <tr className="bg-violet-500/5">
                                      <th className="text-left px-4 py-1.5 text-violet-400/70 font-medium pl-8">
                                        Component
                                      </th>
                                      <th className="text-right px-3 py-1.5 text-violet-400/70 font-medium">
                                        Qty
                                      </th>
                                      <th className="text-right px-3 py-1.5 text-violet-400/70 font-medium">
                                        Unit
                                      ...
                                      <th className="text-right px-4 py-1.5 text-violet-400/70 font-medium">
                                        Line Total
                                      </th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {row.children.map((c, ci) => (
                                      ...
                                      >
                                        <td
                                          className="px-4 py-1.5 text-muted-foreground pl-8 truncate max-w-[200px]"
                                          title={c.description}
                                        >
                                          <span className="text-violet-400/50 mr-1.5">
                                            └
                                          </span>
                                          {c.description}
                                        </td>
                                        ...
                                        <td className="px-3 py-1.5 text-right font-mono text-muted-foreground">
                                          {fmt(c.finalLaborHrs, 3)}
                                        </td>
                                        <td className="px-3 py-1.5 text-right font-mono text-foreground">
                                          ${fmt(c.lineMaterial)}
                                        </td>
                                        <td className="px-4 py-1.5 text-right font-mono font-semibold text-violet-200">
                                          ${fmt(c.lineTotal)}
                                        </td>
                                      </tr>
                                    ))}
                                  ...
                                  <tfoot>
                                    <tr className="border-t border-violet-500/30 bg-violet-500/5">
                                      <td
                                        colSpan={5}
                                        className="px-4 py-1.5 text-xs text-violet-400/70 font-medium pl-8"
                                      >
                                        Assembly Subtotal
                                      </td>
                                      <td className="px-3 py-1.5 text-right font-mono text-sm text-foreground">
                                        ${fmt(row.assemblyMaterial ?? 0)}
                                      </td>
                                      <td className="px-4 py-1.5 text-right font-mono text-sm font-bold text-violet-300">
                                        ${fmt(row.lineTotal)}
                                      </td>
                                    </tr>
                                  </tfoot>
              ...
              {Array.from(rowsByPhase.entries())
                .filter(
                  ([phase]) => phase !== "Unmatched" && phase !== "Assembly"
                )
                .map(([phase, rows]) => {
                  const phaseTotal = rows.reduce((s, r) => s + r.lineTotal, 0);
                  const phaseLaborHrs = rows.reduce(
                    (s, r) => s + (r.finalLaborHours ?? 0) * r.qty,
                    0
                  );
                      ...
                      >
                        <span
                          className={cn(
                            ...
                            PHASE_COLORS[phase] ?? "text-foreground"
                          )}
                        >
                          {phase}
                        </span>
                        ...
                        <span className="text-xs font-mono text-foreground">
                          {fmt(phaseLaborHrs, 1)} hrs
                        </span>
                        <span className="text-xs font-mono font-semibold text-[#F5C518]">
                          ${fmt(phaseTotal)}
                        </span>
                        {isOpen ? (
                          <ChevronUp
                          ...
                          <table className="w-full text-xs">
                            ...
                            <tbody>
                              {rows.map((row, i) => (
                                ...
                                >
                                  <td
                                    className="px-4 py-2 text-foreground max-w-[200px] truncate"
                                  ...
                                  <td className="px-3 py-2 text-right font-mono text-foreground">
                                    ${fmt(row.unitMaterial ?? 0)}
                                  </td>
                                  <td className="px-3 py-2 text-right font-mono text-muted-foreground">
                                    {fmt(row.baseLaborHours ?? 0, 4)}
                                  </td>
                                  <td className="px-3 py-2 text-right font-mono text-[#F5C518]">
                                    {fmt(row.categoryFactor ?? 1, 2)}
                                  </td>
                                  <td className="px-3 py-2 text-right font-mono text-foreground">
                                    {fmt(
                                      (row.finalLaborHours ?? 0) * row.qty,
                                      3
                                    )}
                                  </td>
                            ...
                            <tfoot>
                              <tr className="border-t border-border bg-muted/5">
                                ...
                                <td className="px-4 py-2 text-right font-mono text-sm font-bold text-[#F5C518]">
                                  ${fmt(phaseTotal)}
                                </td>
                              </tr>
                            </tfoot>
                ...
                <div className="bp-card overflow-hidden border-rose-500/30">
                  ...
                  >
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider text-rose-400">
                      Unmatched Items — Manual Review Required
                    </span>
                    <span className="text-xs text-muted-foreground font-mono ml-auto">
                      {unmatchedRows.length} item
                      {unmatchedRows.length !== 1 ? "s" : ""}
                    </span>
                  ...
                  {expandedPhases.has("Unmatched") && (
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-t border-border bg-muted/5">
                            ...
                            <th className="text-left px-4 py-2 text-rose-400 font-medium">
                              Status
                            </th>
                          </tr>
                        </thead>
                      ...
                      <p className="px-4 py-3 text-[11px] text-muted-foreground border-t border-border">
                        These items did not match any entry in the electrical
                        database. Add them manually to the Labor &amp; Material
                        page or expand the database with the exact description.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4 justify-between">
                  <div className="space-y-0.5">
                    ...
                    <div className="text-xs text-muted-foreground font-mono">
                      {category} · Crew ×{crewSlider.toFixed(2)} · ${laborRate}
                      /hr ·{" "}
                      {pricingSource === "platt"
                        ? "Platt Live"
                        : "Mock Pricing"}
                  ...
                  <div className="flex items-center gap-6 text-sm">
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-[10px] text-muted-foreground">
                        Labor ({fmt(totalLaborHours, 1)} hrs)
                      </div>
                    </div>
                    <div className="text-muted-foreground">=</div>
                    <div className="text-center">
                      <div className="font-mono font-bold text-xl text-[#F5C518]">
                        ${fmt(grandTotal)}
                      </div>
                      <div className="text-[10px] text-muted-foreground">
                        Grand Total
